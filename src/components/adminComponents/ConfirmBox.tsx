...
interface CofirmBoxProps {
  close: () => void;
  cancel: () => void;
  confirm: () => void;
}

const CofirmBox: React.FC<CofirmBoxProps> = ({cancel,confirm,close}) => {
  return (
    <div className='fixed top-0 bottom-0 right-0 left-0 z-50 bg-neutral-800 bg-opacity-70 p-4 flex justify-center items-center'>
      <div className='bg-white w-full max-w-md p-4 rounded'>
           ...
           <div className='w-fit ml-auto flex items-center gap-3'>
                ...
                <button onClick={confirm} className='px-4 py-1 border rounded bg-black text-white hover:bg-gray-700 hover:text-white'>Confirm</button>
           </div>
      </div>
    </div>
  )
